...
class ExportFixedDepositUseCase @Inject constructor(private val repository: FixedDepositRepository) {
    suspend fun execute(): String {
        ...
        val header = "ID, Bank Name, Principal Amount, Maturity Amount, Interest Rate, Start Date, Maturity Date, Notes, CreatedAt"
        val csvData = fixedDeposits.firstOrNull()?.joinToString(separator = "\n") { fd ->
            "${fd.id}, ${fd.bankName}, ${fd.principalAmount}, ${fd.maturityAmount}, ${fd.interestRate}, \"${fd.startDate.time.toDateString()}\", \"${fd.maturityDate.time.toDateString()}\", ${fd.notes}, \"${fd.createdAt.time.toDateString()}\""
        }
